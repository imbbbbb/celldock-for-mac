...
**不在三模式之列的配置也能修复**，已实测：一块报告 `2CA3:4006,1,1,1,1,1,0,1`
（大疆出厂身份但开着 USB 音频）的模块，通过切换到任一模式恢复正常。切换写入的是完整的
目标 USBCFG，源配置的标志位不参与写入，因此只要模块的 USB 身份是已知的 QDC507 且 AT
可达，就能被修回来 —— 哪怕它当前的配置从未被验证过。
